...
def add_tags_to_vpcs(region_name):
    ...
    # Leer el nombre de las VPC desde el archivo VPC.txt
    try:
        with open("VPC.txt", "r") as f:
            vpc_ids = [line.strip() for line in f]
    except FileNotFoundError:
        print("Error: Archivo 'VPC.txt' no encontrado.")
        sys.exit(1)

    tag_value = 'connectors-test'  # Reemplazar esto con el valor que necesitas para 'product'

    for vpc_id in vpc_ids:
        # Nuevas etiquetas para cada VPC
        new_tags = [
        ...
        # Agregar las nuevas etiquetas a cada VPC
        response = ec2_client.create_tags(
            Resources=[vpc_id],
            Tags=new_tags
        )

        print(f"Etiquetas agregadas a la VPC '{vpc_id}': {response}")
