...
def add_tags_to_elastic_ips(region_name):
    ...
    # Leer la lista de IPs elásticas desde un archivo (eip_Addresses.txt)
    try:
        with open("eip_Addresses.txt", "r") as f:
            elastic_ips = [line.strip() for line in f]
    ...
    for ip in elastic_ips:
        try:
            # Obtener el AllocationId de la dirección IP elástica
            response = ec2_client.describe_addresses(Filters=[{'Name': 'public-ip', 'Values': [ip]}])
            allocation_id = response['Addresses'][0]['AllocationId']

            # Nuevas etiquetas para cada IP elástica
            new_tags = [
            ...
            # Agregar las nuevas etiquetas a la IP elástica
            response = ec2_client.create_tags(
                Resources=[allocation_id],
                Tags=new_tags
            )
            print(f"Etiquetas agregadas a la IP elástica '{ip}': {response}")
        except Exception as e:
            print(f"Error al etiquetar la IP elástica '{ip}': {e}")
...
if __name__ == "__main__":
    ...
    add_tags_to_elastic_ips(region_name)
